        ...
        if check_time.weekday() >= 5:  # Weekend
            ...
            if check_time.time() >= self.market_session.market_open:
                ...
                if next_market_open.weekday() >= 5:  # If it's weekend, go to Monday
                    ...
                    next_market_open += timedelta(days=days_to_monday)
...
if __name__ == "__main__":
    ...
    print(f"Current IST: {tu.now_ist()}")
    print(f"Current UTC: {tu.now_utc()}")
    print(f"CSV Format: {now_csv_format()}")
    ...
    test_timestamps = [
        "2025-08-24 14:30:00",
        "2025-08-24T14:30:00+05:30",
        "2025-08-24T09:00:00Z",
        1724504400.0
    ]
    
    print("\n=== Timestamp Standardization ===")
    for ts in test_timestamps:
        standardized = standardize_timestamp_column(ts)
    ...
    print(f"Minute bucket: {minute_bucket(now)}")
    
    # Test market session buckets for today
    today = tu.get_current_date_ist()
    if today.weekday() < 5:  # Only on weekdays
        buckets = tu.get_minute_buckets_for_session(today)
        print(f"Total market minutes today: {len(buckets)}")
        print(f"First few buckets: {buckets[:5]}")
        print(f"Last few buckets: {buckets[-5:]}")